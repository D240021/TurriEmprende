...
export function EntrepreneurshipDetailsPage() {
  ...
  useEffect(() => {
    const foundEntrepreneurship = entrepreneurshipsData.find(e => e.id === id);
        ...
        <div className="md:flex">
          ...
          <div className="p-6 md:w-1/2">
            ...
            <div className="border-t border-b border-gray-200 py-4 my-4">
              <p className="text-gray-700 mb-4">{entrepreneurship.description}</p>
              <h3 className="font-semibold text-lg mb-2">Información de contacto</h3>
              <div className="space-y-2">
                <p className="flex items-center text-gray-700">
                  <PhoneIcon className="w-5 h-5 mr-2 text-green-600" /> {entrepreneurship.phone}
                </p>
                <p className="flex items-center text-gray-700">
                  <MailIcon className="w-5 h-5 mr-2 text-green-600" /> {entrepreneurship.email}
                </p>
                <p className="flex items-center text-gray-700">
                  <MapPinIcon className="w-5 h-5 mr-2 text-green-600" /> {entrepreneurship.address}
                </p>
                <p className="flex items-center text-gray-700">
                  <ClockIcon className="w-5 h-5 mr-2 text-green-600" /> {entrepreneurship.hours}
                </p>
              </div>
            </div>
            <div className="mt-4">
              ...
              <ul className="list-disc list-inside space-y-1 text-gray-700">
                {entrepreneurship.products.map((product, index) => (
